...
async def create_incident(monitor_id: UUID, data: CreateIncident):
    async with get_db() as db:
        return await IncidentService(db).create(monitor_id, data)
...
async def search_incidents(
    ...
    async with get_db() as db:
        return await IncidentService(db).search(monitor_id, filters)


@incident_router.get("/{monitor_id}/{id_}", response_model=IncidentResponse)
async def get_incident(monitor_id: UUID, id_: UUID):
    async with get_db() as db:
        return await IncidentService(db).get_by_id(monitor_id, id_)


@incident_router.put("/{monitor_id}/{id_}", response_model=IncidentResponse)
async def update_incident(monitor_id: UUID, id_: UUID, data: UpdateIncident):
    async with get_db() as db:
        return await IncidentService(db).update(monitor_id, id_, data)


@incident_router.delete("/{monitor_id}/{id_}", status_code=204)
async def delete_incident(monitor_id: UUID, id_: UUID):
    async with get_db() as db:
        await IncidentService(db).delete(monitor_id, id_)
